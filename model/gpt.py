...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.position_embed = nn.Embedding(context_length, model_dim)
        self.tf = nn.Sequential(*[self.TransformerBlock (model_dim, num_heads) for _ in range(num_blocks)])
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        token_emb = self.word_embed(context)  # [B, T, C]
        pos_ids = torch.arange(seq_len, device=context.device)  # [T]
        pos_emb = self.position_embed(pos_ids).unsqueeze(0)     # [1, T, C]
        ...
        # Return rounded logits (no softmax)
